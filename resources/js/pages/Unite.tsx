import UniteFormModal from '@/components/unite/UniteFormModal';
import AppLayout from '@/layouts/app-layout';
import { _Admission, _Chambre, _Hopital, type _Unite } from '@/lib/types';
...
const UniteCard = ({ unite, onEdit }: { unite: _Unite; onEdit: (unite: _Unite) => void }) => {
            ...
            >
                ...
                <div className="flex items-center gap-2">
                    <div className="flex gap-2">
                        ...
                        <Link
                            href={route('api.unites.destroy', unite.code)}
                            method="delete"
                            as="button"
                            onClick={() => window.location.reload()}
                            className="rounded-full p-2 text-red-600 transition-all duration-300 ease-in-out hover:scale-110 hover:cursor-pointer hover:bg-red-50 active:scale-95"
                        >
...
export default function Unites() {
    const { unites, hopitals } = usePage<UniteProps>().props;
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [editingUnite, setEditingUnite] = useState<_Unite | undefined>(undefined);

    const openCreateModal = () => {
        setEditingUnite(undefined);
        setIsModalOpen(true);
    };

    const openEditModal = (unite: _Unite) => {
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="container mx-auto px-4 py-8">
                ...
                <div className="grid grid-cols-1 gap-6">
                    {unites.map((unite) => (
                        <UniteCard key={unite.code} unite={unite} onEdit={openEditModal} />
                    ))}
                </div>
            ...
            <UniteFormModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} unite={editingUnite} hopitals={hopitals} />
        </AppLayout>
    );
}
